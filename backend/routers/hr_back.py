...
def _build_pe_docx(user, req: PERequest) -> bytes:
    ...
    doc = Document(str(tp)); dash = "—"; today = date.today()
    mapping = {
        "{{NOM_PRENOM}}": user.nom_prenom, "{{MATRICULE}}": user.matricule,
        "{{UNITE}}": user.unite, "{{DESTINATAIRE}}": user.destinataire,
        "{{LIEU}}": "Casablanca", "{{DATE_CREATION}}": today.strftime("%d/%m/%Y"),
...
def regenerate_pe(doc_id: int, current_user=Depends(auth.get_current_user), db: Session=Depends(get_db)):
    ...
    req = PERequest(type_perm=meta.get("type_perm","Heures"),date1=meta.get("date1"),
                    date2=meta.get("date2"),heure_debut=meta.get("heure_debut"),heure_fin=meta.get("heure_fin"))
...
def _build_sortie_docx(user, req: SortieRequest) -> bytes:
    ...
    mapping = {
        ...
        "{{HEURE1}}": fmt_heure(req.heure1), "{{HEURE2}}": fmt_heure(req.heure2),
        "{{MOTIF}}": req.motif, "{{DATE}}": date.today().strftime("%d/%m/%Y"),
    }
    replace_placeholders(doc, mapping)
    buf=io.BytesIO(); doc.save(buf); buf.seek(0); return buf.read()
...
def regenerate_sortie(doc_id: int, current_user=Depends(auth.get_current_user), db: Session=Depends(get_db)):
    ...
    original = get_original_user(db, record, current_user)
    meta = record.metadata_doc
    req = SortieRequest(jour=meta["jour"],heure1=meta["heure1"],heure2=meta["heure2"],motif=meta["motif"])
...
def regenerate_reprise(doc_id: int, current_user=Depends(auth.get_current_user), db: Session=Depends(get_db)):
    ...
    original = get_original_user(db, record, current_user)
    meta = record.metadata_doc
    req = RepriseRequest(date_reprise=meta["date_reprise"],adress_medcin=meta.get("adress_medcin",""))
...
def regenerate_maladie(doc_id: int, current_user=Depends(auth.get_current_user), db: Session=Depends(get_db)):
    ...
    original = get_original_user(db, record, current_user)
    meta = record.metadata_doc
    req = MaladieRequest(date_maladie=meta["date_maladie"],date_reprise=meta["date_reprise"],
...
def regenerate_fin_manquant(doc_id: int, current_user=Depends(auth.get_current_user), db: Session=Depends(get_db)):
    ...
    original = get_original_user(db, record, current_user)
    meta = record.metadata_doc
    req = FinManquantRequest(date_manquant=meta["date_manquant"],heure_manquant=meta["heure_manquant"])
...
def regenerate_rc(doc_id: int, current_user=Depends(auth.get_current_user), db: Session=Depends(get_db)):
    ...
    original = get_original_user(db, record, current_user)
    meta = record.metadata_doc
    req = RCRequest(service=meta["service"],nomber_jours=meta["nomber_jours"],
